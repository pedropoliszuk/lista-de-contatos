...
function App() {
  ...
  const { listaContatos, contatoSelecionado } = useSelector(
    (state: RootState) => state.contatos
  )

  const [contato, setContato] = useState({
    id: listaContatos.length + 1,
    nome: '',
    telefone: '',
    email: ''
  })
      ...
      <AppContainer>
        ...
        <div>
          ...
          <ContatoInput
            ...
            onChange={(e) => handleInputChange(e, false)}
          />
          <BotaoAdicionar onClick={adicionarContatoHandler}>
            Adicionar contato
          </BotaoAdicionar>
        </div>
        <div>
          {listaContatos.map((cont) => (
            <div key={cont.id}>
              <Contato
                ...
                email={cont.email}
                onDelete={deletarContatoHandler}
                onEdit={editarContatoHandler}
              />
              <BotaoEditar onClick={() => editarContatoHandler(cont.id)}>
                Editar
              </BotaoEditar>
              <BotaoDeletar onClick={() => deletarContatoHandler(cont.id)}>
                Deletar
              </BotaoDeletar>
            </div>
          ))}
        ...
        {contatoSelecionado && (
          <div>
            ...
            <ContatoInput
              ...
              onChange={(e) => handleInputChange(e, true)}
            />
            <BotaoSalvarEdicao onClick={salvarEdicaoHandler}>
              Salvar Edição
            </BotaoSalvarEdicao>
            <BotaoDeletar onClick={() => dispatch(selecionarContato(null))}>
              Cancelar
            </BotaoDeletar>
          </div>
        )}
